...
func (q *Queries) CreateUser(ctx context.Context, arg CreateUserParams, getUserByID *int32, getUserByName *string) (*User, error) {
	row := q.db.WQueryRow(ctx, "users.CreateUser", createUser, arg.Name, arg.Metadata, arg.Thumbnail)
	var i *User = new(User)
	err := row.Scan(
		&i.ID,
		&i.Name,
...
func (q *Queries) Dump(ctx context.Context, beforeDump ...BeforeDump) ([]byte, error) {
	sql := "SELECT id,name,metadata,thumbnail,createdat FROM users ORDER BY id,name,thumbnail,createdat ASC;"
...
func (q *Queries) Load(ctx context.Context, data []byte) error {
	...
	for _, row := range rows {
		_, err := q.db.WExec(ctx, "users.Load", sql, row.ID, row.Name, row.Metadata, row.Thumbnail, row.CreatedAt)
		if err != nil {
			return err
		}
	}
